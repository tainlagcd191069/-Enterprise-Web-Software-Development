...
const deletePostQA = async (req, res, next) => {
  const { postId } = req.params;
  let post;
  try {
    //"populate" allows us to refer to another collection and work with it
...
const compareDate = async (req, res, next) => {
  const dateFrom =  req.params.fromDate;
  const dateTo = req.params.toDate;
  try {
...
exports.deletePostQA = deletePostQA;
